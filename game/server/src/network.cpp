...
void network::init(int port)
{
	g_networkThread = std::thread([port]()
	{
		...
		app.ws<int>("/game", {
			.open = [](uWS::WebSocket<false, true, int> int> *ws)
			{
				std::cout << "[NETWORK] Client connected\n";
				std::lock_guard<std::mutex> lock(g_clientsMutex);
				g_clients.push_back(ws);
			},
			.message = [](uWS::WebSocket<false, true, int> *ws,
			...
			.close = [](uWS::WebSocket<false, true, int> *ws,
						int code,
						std::string_view message)
			{
				(void)code;
				...
				g_clients.erase(
					std::remove(g_clients.begin(), g_clients.end(), ws),
					g_clients.end()
				);
			}
		})
		.get("/heath", [](uWS::HttpResponse<false> *res, uWS::HttpRequest *)
		{
			res->end("OK");
		})
...
void network::broadcast(const std::string &json)
{
	if (!g_loop)
		return;
